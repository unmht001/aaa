        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
            Container(
                ...
                child: FlatButton(child: Icon(Icons.close), onPressed: () => Navigator.of(context).pop())),
            Text("筛选", style: TextStyle(decoration: TextDecoration.none, fontSize: 35, color: Colors.grey[800])),
            Container(width: 50)
          ]),
...
Widget getlabel(String s1, String s2, {Color cl: Colors.white}) {
  return Container(
      ...
      decoration: BoxDecoration(
          color: cl, border: Border.all(width: 2, color: cl), borderRadius: BorderRadius.all(Radius.circular(5.0))),
      child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: <Widget>[stext(s1 ?? ""), stext(s2 == "null" ? "" : s2)]));
}

List<Widget> bookCellDataList2WidgetList(List<Blockcelldata> bd2, int grouped) {
...
Widget makeblock(List<Blockcelldata> bd, String title, int grouped) {
  ...
  bd2 = bd.sublist(0);
  if (bd2.length % grouped > 0) for (var i = bd2.length % grouped; i < grouped; i++) bd2.add(Blockcelldata(null, null));
  ...
  return Column(children: <Widget>[
    ...
    Container(
        ...
        padding: EdgeInsets.all(5),
        child: Column(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: bookCellDataList2WidgetList(bd2, grouped)))
  ]);
}
