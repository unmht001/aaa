...
fn1() async {
  // Map m = {"searchtype": "all", "searchkey": "地球", "sbt": "搜索"};
  ...
  try {
    ...
    var url =
        "http://www.shumil.co/search.php?all&searchkey=%B5%D8%C7%F2&sbt=%CB%D1%CB%F7";

    Response response = await dio.get(url);

    if (response.statusCode == 200) {
      var s = charsetS(response, charset: "gbk").toString();
      s = RegExp("<div\\sclass=\"list\">([\\S\\s]+?)<div\\sclass=\"clear\">",
              multiLine: true)
          .firstMatch(s)
          .group(1)
          .toString();
      ...
      _ret = s2
          .toList()
          .map((vs) => [
                (_r = s12.firstMatch(vs.group(1).toString()))
                    .group(1)
                    .toString(),
                _r.group(2).toString()
              ])
